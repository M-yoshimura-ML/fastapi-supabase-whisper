...
class OpenAIController:
    ...
    def _add_routes(self):
        ...
        async def chat(data: ChatRequest, current_user: User = Depends(get_current_user)):
            reply = await self.openai_service.chat_with_history(data)
        ...
        async def text_chat(
            ...
            try:
                ...
                reply = await self.openai_service.chat_with_history(payload)
                logger.info(f"🧠 OpenAI response time: {time.time() - openai_start:.2f} sec")

                # Translate
                translate_start = time.time()
                translated_text = await self.openai_service.translate_text(reply, payload.language)
                logger.info(f"🌍 Translation time: {time.time() - translate_start:.2f} sec")

                # TTS
                tts_start = time.time()
                mp3_file = await self.openai_service.generate_openai_tts(reply)
        ...
        async def voice_chat(
            ...
            try:
                ...
                # 1. Transcribe
                user_text = await self.openai_service.transcribe(audio_file, language)
                logger.info(f"🧠 OpenAI transcribe time: {time.time() - transcribe_start:.2f} sec")

                chat_start = time.time()
                # 2. Chat
                reply_text = await self.openai_service.chat_with_text(user_text)
                logger.info(f"🧠 OpenAI chat time: {time.time() - chat_start:.2f} sec")

                tts_start = time.time()
                # 3. TTS
                speech_response = await self.openai_service.tts(reply_text)
